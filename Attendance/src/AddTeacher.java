...
public class AddTeacher extends JFrame{
	...
	AddTeacher() throws SQLException, ClassNotFoundException{
		super("Add Teacher");
        ...
        res = st.executeQuery("select user from login where desg='T';");
        while (res.next())
        ...
        pane.add(new JLabel("Teacher Name"));
        pane.add(user);
        ...
        addinfo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					{
						String sql = "insert into login values('"+userText+"', '"+passText+"', 'T');";
						st.executeUpdate(sql);
						users.add(userText);
						JOptionPane.showMessageDialog(null, "Teacher Added", "Success", JOptionPane.INFORMATION_MESSAGE);
					}
					else {
